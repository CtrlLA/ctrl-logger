...
if (process.env.logLevel) {
  defaults.level = parseInt(process.env.logLevel);
  ...
  for (var i = 4, len = arguments.length; i < len; i++) {
    if (arguments[i]) {
      if (typeof arguments[i] === 'string') {
        ...
        args.push(nodeUtil.inspect(arguments[i], {showHidden: true, depth: 3}));
        output.push(nodeUtil.inspect(arguments[i], {showHidden: true, depth: 3}));
      }
    }
  }
